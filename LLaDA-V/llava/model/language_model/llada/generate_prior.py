    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            if cfg_scale > 0.:
                ...
                # ==== [추가 시작] prior bias correction (orthogonalization → logit subtraction) ====
                with torch.no_grad():
                    print("Applying prior bias correction...")
                    print(model)
                    ...
                    out_prior = model(
                        None,
                        input_embeddings=E_mean.unsqueeze(0),
                        # output_hidden_states=True,
                        # return_dict=True,
                    )

                    out_prior_logits = out_prior.logits 
                    logp_prior = (
                        out_prior_logits[0, -1]
                        .log_softmax(dim=-1)
                        .to(next(model.parameters()).device)
                    )

                # (3) Logit-level prior subtraction
                alpha = 0.2
                logits = logits - alpha * logp_prior.view(1, 1, -1)
                # ==== [추가 끝] ====
            # logits = logits.cpu()
