...
def generate(model, prompt=None, steps=None, max_new_tokens=128, block_length=128, temperature=0.,
    ...
    def init_prior_dir(model):
        # Cache the prior direction on the model so this computation runs only once per model instance.
        if hasattr(model, '_cached_prior_dir') and model._cached_prior_dir is not None:
            return model._cached_prior_dir
        ...
        h_prior = model.transformer.ln_f(out.hidden_states[-1])[0,0]
        prior_dir = F.normalize(h_prior, dim=0)

        # Store on model for future calls
        try:
            model._cached_prior_dir = prior_dir
        except Exception:
            # If the model object doesn't allow setting attrs, skip caching silently.
            pass
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            if cfg_scale > 0.:
                ...
                if prefix_lm:
                    # breakpoint()
                    outputs = model(None,input_embeddings=inputs_embeds_curr,past_key_values=past_key_values, rope=rope, mode=mode, \
                        output_hidden_states=True)
                else:
                    if inputs_embeds is not None:
                        inputs_embeds_curr[:,:inputs_embeds.shape[1]] = inputs_embeds
                    outputs = model(None,input_embeddings=inputs_embeds_curr, rope=rope, mode=mode, output_hidden_states=True)
                # 추가 ===============
                prior_dir = init_prior_dir(model)
                d=torch.nn.functional.normalize(prior_dir,dim=0).to(model.device)

                H_full = outputs.hidden_states[-1]  # (B=1, L, D)
                mask_index = (x == mask_id)         # (1, L)

                masked_pos = mask_index[0].nonzero(as_tuple=False).squeeze(1)   # (M,)
                if masked_pos.numel() > 0:
                    H_masked = H_full[0, masked_pos]                            # (M, D)

                    cos_sim = F.cosine_similarity(H_masked, d.unsqueeze(0), dim=-1)  # (M,)
                    keep_mask = cos_sim >= 0.0

                    if keep_mask.any():
                        sel_local = keep_mask.nonzero(as_tuple=False).squeeze(1)     # (K,)
                        sel_global = masked_pos[sel_local]                            # (K,)  <-- IMPORTANT

                        H_sel = H_full[0, sel_global]                                 # (K, D)

                        alpha_d = prior * torch.relu(cos_sim[sel_local])              # (K,)
                        proj_d = (H_sel @ d).unsqueeze(-1) * d.unsqueeze(0)           # (K, D)

                        H_temp_sel = H_sel - alpha_d.unsqueeze(-1) * proj_d           # (K, D)
                        H_full[0, sel_global] = H_temp_sel.to(H_full.dtype)           # write back
                logits = H_full @ model.transformer.ff_out.weight.t()
                #====#
